...
def rCreate(pcode, p):
    ...
    rDict[label] = [rPush,
                    heapNext]  # when created word is run, pushes its address


def rDoes(cod, p):
    rDict[lastCreate] += cod[
                         p:]  # rest of words belong to created words runtime
...
def cThen(pcode):
    ...
    code, slot = cStack.pop()
    if code not in ("IF", "ELSE"): fatal(
        "THEN preceded by %s (not IF or ELSE)" % code)
